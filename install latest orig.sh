...
existing_openai="$(get_existing_json_value secrets.openaiApiKey "$CONFIG_FILE")"
existing_telegram="$(get_existing_json_value channels.telegram.token "$CONFIG_FILE")"

echo
prompt_required OPENAI_API_KEY "Enter OPENAI_API_KEY" "$existing_openai"
prompt_required TELEGRAM_BOT_TOKEN "Enter TELEGRAM_BOT_TOKEN" "$existing_telegram"

echo
if [[ -f "$CONFIG_FILE" ]]; then
  read -r -p "Found existing servclaw.json. Overwrite keys with provided values? [y/N]: " overwrite
  overwrite="${overwrite:-N}"
  if [[ ! "$overwrite" =~ ^[Yy]$ ]]; then
    log "Skipped writing servclaw.json."
  else
    umask 077
    python3 - "$CONFIG_FILE" "$OPENAI_API_KEY" "$TELEGRAM_BOT_TOKEN" <<'PY'
import json, sys
cfg_file, openai_key, telegram_token = sys.argv[1:4]
try:
    ...
    cfg = {}
cfg.setdefault('meta', {})
cfg['meta'].setdefault('lastTouchedVersion', '2026.3.18')
cfg.setdefault('agents', {}).setdefault('defaults', {}).setdefault('model', {}).setdefault('primary', 'openai/gpt-5-mini')
cfg.setdefault('channels', {}).setdefault('telegram', {})
cfg['channels']['telegram'].setdefault('enabled', True)
cfg['channels']['telegram']['token'] = telegram_token
cfg['channels']['telegram'].setdefault('allowedUserIds', [])
cfg['channels']['telegram'].setdefault('streaming', 'off')
